...
class VigenereCipherWithPlainTextAutokeyCipher(AlgorithmBase):
    # region Реализация AlgorithmBase
    def get_name(self):
        return "Шифр Виженера с самоключом по открытому тексту"

    def get_description(self):
        return "Зашифровывает и расшифровывает строку через Шифр Виженера с самоключом по открытому тексту."
    ...
    def encrypt(self, alphabet: str, plain_text: str, key_text: str):
        return self.encrypt_via_vigenere_cipher_with_plain_text_autokey_cipher(
            alphabet, plain_text, key_text
        )

    def decrypt(self, alphabet: str, cipher_text: str, key_text: str):
        return self.decrypt_via_vigenere_cipher_with_plain_text_autokey_cipher(
    ...
    def encrypt_via_vigenere_cipher_with_plain_text_autokey_cipher(
        self, alphabet: str, plain_text: str, key_text: str
    ):
        """
        Зашифровывает строку шифром Виженера с самоключом по открытому тексту.

        :param alphabet: Алфавит, используемый для шифрования
        :type alphabet: str
        :param plain_text: Исходная строка для зашифрования
        :type plain_text: str
        :param key_text: Начальный лозунг (пароль)
        :type key_text: str
        ...
        key_array = list(key_text)
        result = []

        for i in range(len(plain_text)):
            if i >= len(key_array):
                key_array.append(plain_text[i - len(key_text)])

            result.append(alphabet[(index_map[plain_text[i]] + index_map[key_array[i]]) % len_of_alphabet])

        return "".join(result)

    def decrypt_via_vigenere_cipher_with_plain_text_autokey_cipher(
        self, alphabet: str, cipher_text: str, key_text: str
    ):
        """
        Расшифровывает строку, зашифрованную шифром Виженера с самоключом по открытому тексту.

        :param alphabet: Алфавит, используемый для расшифрования
        :type alphabet: str
        :param cipher_text: Зашифрованная строка
        :type cipher_text: str
        :param key_text: Начальный лозунг (пароль), использованный при зашифровании
        :type key_text: str
        ...
        for i in range(len(cipher_text)):
            if i >= len(key_array):
                key_array.append(result[i - len(key_text)])

            result.append(alphabet[(index_map[cipher_text[i]] - index_map[key_array[i]])% len_of_alphabet])
