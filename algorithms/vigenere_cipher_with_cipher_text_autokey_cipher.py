...
class VigenereCipherWithCipherTextAutokeyCipher(AlgorithmBase):
    # region Реализация AlgorithmBase
    def get_name(self):
        return "Шифр Виженера с самоключом по шифртексту"

    def get_description(self):
        return "Зашифровывает и расшифровывает строку через Шифр Виженера с самоключом по шифртексту."
    ...
    def encrypt(self, alphabet: str, plain_text: str, key_text: str):
        return self.encrypt_via_vigenere_cipher_with_cipher_text_autokey_cipher(
            alphabet, plain_text, key_text
        )

    def decrypt(self, alphabet: str, cipher_text: str, key_text: str):
        return self.decrypt_via_vigenere_cipher_with_cipher_text_autokey_cipher(
    ...
    def encrypt_via_vigenere_cipher_with_cipher_text_autokey_cipher(
        self, alphabet: str, plain_text: str, key_text: str
    ):
        """
        Зашифровывает строку шифром Виженера с самоключом по шифртексту.
    ...
    def decrypt_via_vigenere_cipher_with_cipher_text_autokey_cipher(
        self, alphabet: str, cipher_text: str, key_text: str
    ):
        """
        Расшифровывает строку, зашифрованную шифром Виженера с самоключом по шифртексту.
        ...
        for i in range(len(cipher_text)):
            if i >= len(key_array):
                key_array.append(cipher_text[i - len(key_text)])
