...
class HillCipher(AlgorithmBase):
    # region Реализация AlgorithmBase
    def get_name(self):
        return "Шифр Хилла"

    def get_description(self):
        return "Зашифровывает и расшифровывает строку через Шифр Хилла."

    def get_arguments_to_setup(self):
        return {
            "Алфавит M": self.set_alphabet,
            "Строка": self.validate_text,
            "Ключ-матрица K (в формате \"[[a11, a12], [a21, a22]]\")": self.validate_key_matrix,
            "Пустой символ": self.validate_char
        }

    def encrypt(self, alphabet: str, plain_text: str, k_matrix: ndarray, secret_char: str):
        return self.encrypt_via_hill_cipher(
            alphabet, plain_text, k_matrix, secret_char
        )

    def decrypt(self, alphabet: str, cipher_text: str, k_matrix: ndarray, secret_char: str ):
        return self.decrypt_via_hill_cipher(
            alphabet, cipher_text, k_matrix, secret_char 
        )
    ...
    def encrypt_via_hill_cipher(
        self, alphabet: str, plain_text: str, k_matrix: ndarray, secret_char: str 
    ):
        """
        Зашифровывает строку шифром Хилла
        """
        len_of_alphabet = len(alphabet)
        k_matrix_size = k_matrix.shape[0]
        remainder = len(plain_text) % k_matrix_size
        if remainder != 0:
            plain_text += secret_char*(k_matrix_size - remainder)

        index_map = {ch: i for i, ch in enumerate(alphabet)}
        result = []
        

        for i in range(len(plain_text) // k_matrix_size):
            block = [index_map[x] for x in plain_text[i*k_matrix_size:(i+1)*k_matrix_size]]
    ...
    def decrypt_via_hill_cipher(
        self, alphabet: str, cipher_text: str, k_matrix: ndarray, secret_char: str 
    ):
        """
        Расшифровывает строку, зашифрованную шифром Хилла.
        """
        len_of_alphabet = len(alphabet)
        k_matrix_size = k_matrix.shape[0]

        if len(cipher_text)%k_matrix_size != 0:
            raise ValueError("Ошибка - длина шифртекста должна быть кратна рангу ключа матрицы.")
        
        inv_matrix = self.get_inv_mod_matrix(k_matrix, len_of_alphabet)

        index_map = {ch: i for i, ch in enumerate(alphabet)}
        result = []

        for i in range(len(cipher_text) // k_matrix_size):
            block = [index_map[x] for x in cipher_text[i*k_matrix_size:(i+1)*k_matrix_size]]
            X_matrix = np.array(block).T
            res_vector = (inv_matrix @ X_matrix) % len_of_alphabet
            result.extend([alphabet[int(round(res)) % len_of_alphabet] for res in res_vector])
